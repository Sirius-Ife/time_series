---
title: 'pallet prices : Time Series Forecast'
author: "sirius_ife"
date: "2024-04-22"
output:
...
```{r}
df<- read.csv('pallet_prices.csv')
colnames(df)<-c("date", "price")
df$date<-as.Date(df$date, format="%Y-%m-%d")
palletXTS<- xts(df[-1], df[[1]])
palletTS<- ts(df$price, start=c(1972,1),end=c(2017,8), frequency=12 )
```


**Trend**
```{r}
ts_plot(palletXTS, title="pallet price Time Series")
ts_decompose(palletTS)
```
...
ts_heatmap(palletTS)
ts_surface(palletTS)
ts_seasonal(palletTS, type="cycle")
ts_seasonal(palletTS, type="box")
```
The plots suggest annual seasonality. We can observe higher price on october, november and december. 
...
```{r}
train<-window(palletTS, start=c(1972,1), end=c(2009,12))
test<-window(palletTS, start=c(2010,1))
```
...
xlab('Year')+ylab('pallet price')+
ggtitle('Forecasts for pallet price')+
...
xlab('Month/Year')+ylab('pallet price')+
ggtitle('Forecasts for pallet price')+
guides(colour=guide_legend(title='Forecast'))
```
